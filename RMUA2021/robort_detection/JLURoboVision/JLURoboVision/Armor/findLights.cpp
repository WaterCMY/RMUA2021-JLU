...
void ArmorDetector::findLights() {
	...
	for (const auto& lightContour : lightContours) {
		...
		if (contourArea(lightContour) < armorParam.min_area || contourArea(lightContour) > armorParam.max_area) continue; //minarea of lightContour to filter some small blobs 面积筛选滤去小发光点
		lightRect = fitEllipse(lightContour); //lightContour fits into a RotatedRect 拟合椭圆

		if (MIN(lightRect.size.height, lightRect.size.width) / MAX(lightRect.size.height, lightRect.size.width) > 0.6) continue;//强识别下灯条长宽比
		if (MAX(lightRect.size.height, lightRect.size.width) > armorParam.max_length) continue;

        lightRect.center.x += roiNext[2];
        lightRect.center.y += roiNext[0];
    ...
    if (lights.size() < 4 ) {
		for (const auto& lightContour : lightContours) {
			if (lightContour.size() < 6) continue; //if contour's size is less than 6 , then it can not used to fitEllipse 轮廓点数小于6，不可拟合椭圆
			if (contourArea(lightContour) < armorParam.min_area_weak || contourArea(lightContour) > armorParam.max_area)  continue;
			//if (MIN(lightRect.size.height, lightRect.size.width) / MAX(lightRect.size.height, lightRect.size.width) > 0.7) continue;//弱识别下灯条长宽比
			lightRect = fitEllipse(lightContour); //lightContour fits into a RotatedRect 拟合椭圆
			if (MAX(lightRect.size.height, lightRect.size.width) > armorParam.max_length) continue;
			...
			if (abs(light.angle) > armorParam.max_angle_weak) continue; //angle filter 角度筛选，滤去一些竖直偏角偏大的
			lights.emplace_back(light);
		}
