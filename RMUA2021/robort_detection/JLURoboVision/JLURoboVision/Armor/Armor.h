...
struct ArmorParam{
    ...
    float max_area;		// min area of light bar 灯条允许的最大面积
    float min_area_weak;// min area of light bar in weakly detect 弱搜索下灯条允许的最小面积
    float max_angle;	//max angle of light bar 灯条允许的最大偏角
    float max_angle_weak;//min area of light bar in weakly detect 弱搜索下灯条允许的最大偏角
...
struct DataBefore
{
    RelativePos r_pos_;         //relative position of target armor 
    ArmorBox target_armor_;     //target armor
    ShootState shoot_;          //the shoot state of this frame
    int armor_quantity_[6];     //the quantity of diffirent car's armor
    int following_num_;         //the armor num trying to follow
    int jump_cnt_;              //how many times has the lights jump 1sec before
    int if_jump_;               //if this fram jump
    int follow_index_;          //counting how many frames has been threw since follow the same target 
    int follow_car_index_;      //counting how many frames has been threw since follow the same car
};
...
public:
    ...
    */
    ...
    /**
     *@brief: return the Detector status 识别程序是否识别到装甲版
     *@return: LOST(0) WAITING(1) FOLLOWING(2) SPINNING(3) ARMOR_SWITCH(4) CAR_SWITCH(5)
     */
    int isShoot();
     ...
     */
    void showDebugInfo(bool showSrcImg_ON, bool showSrcROI_ON, bool showSrcBinary_ON, bool showLights_ON, bool showArmors_ON, bool textLights_ON, bool textArmors_ON, bool textScores_ON);

    void setPlayerNum(const int & targetNum);
...
private:
    ...
    ShootState shoot; //the state of target
    vector<DataBefore> datas; //store the identify result of some frames before to judge the shooting state 
    ArmorBox waiting_armor_;
    ArmorBox stable_armor_;
    int targetNum; //the target num tring to follow now当前尝试跟随的装甲板数字
    int playerNum;
    int lost_cnt_; //counting the frames lost target
    int wait_cnt_; //counting the frames waiting target
    int follow_cnt_; //counting the frames following the same car
    int switch_armor_cnt_; //counting switch armor
    int roiNext[4];//the roi next frame is going to detect[rows1, rows2, cols1, cols2] 下一帧的ROI区域，左上角点的坐标x,y和区域行列
    int last_jump_index_;
    Point2f  predict_spinning_;
};
