...
//#define HIGH  0.88
#define SHOOTERLENTH 0.18
//视觉用 计算敌人当前位置

enemy_t*  Enemy_CalNow(double X,double Y, double Z, vision_pr* attitude, Filter& filter_)
{
//    printf("!!!!yaw_pre : %f\t",attitude->yaw_now);
//    printf("!!!!pitch_pre : %f\t",attitude->pitch_now);
//    printf("!!!!Bullet_speed : %f\n",attitude->Bullet_speed);
...
//    printf("！！！################enemyattitude->yaw_pre: %f \n", attitude->yaw_now);
//    printf("！！！################asin(enemy.x/enemy.horizontal_dis): %f \n",asin(enemy.x/enemy.horizontal_dis));
//    printf("！！！################enemy.angle360: %f \n", enemy.angle360);
//    printf("！！！################asin(enemy.horizontal_dis): %f \n",enemy.horizontal_dis);
...
//    printf("！！！################X: %f \n", X);
//    printf("！！！################enemy.x: %f \n", enemy.x);
//    printf("！！！################enemy.last_x: %f \n", enemy.last_x);
//    printf("！！！&&&&&&&&&&&&&&&&&&&&&&&enemy.x_filter: %f\n",enemy.x_filter);
//    printf("！！！&&&&&&&&&&&&&&&&&&&&&&&enemy.vx_filter: %f\n",enemy.vx_filter);
...
//    return enemy;
   ...
   if (armor_change == 3 || find_num <= 15)
   {
    ...
    shoot_delay = ( enemy->distance / shoot_speed ) * 25.0;
    mech_delay = 3.0;

    printf( " ))))))))))))))))))))))))))))))))))))))))shoot_delay : %f", shoot_delay);
...
//        *pitch = -(asin(save_enemy->Z/save_enemy->horizontal_dis)*180.0)/3.14159;
        printf("______________________________high : %f\t", high_comp);
//        if (*pitch>17)
//        {
...
//    printf( " speed:::::::::::::: %f" ,speed_h);
//    //printf("@@speed @@: %f", speed);
//    derta =(high*9.8f+speed_h*speed_h)*(high*9.8f+speed_h*speed_h)-9.8f*9.8f*distance*distance;
//    t = sqrt((( (high*9.8f)+ (speed* speed) )-sqrt(derta))/(0.5f*9.8f*9.8f));
//    v_x =  sqrt((distance*distance-(high*high))/t);
//    v_y = ((high-0.5*9.8*t*t)/t);
//    pitch = -(atan(v_y/v_x)/3.14159)*180.0;
...
////    }
//    return pitch;
//// = (((8192.0f*((acos(v_x/(speed_h)))-2.5f))/(360.0f)) +6830);
    float t,derta,v_x,pitch, v_y;
    float speed_h;
    speed_h = 15;
    if(speed!=0)
