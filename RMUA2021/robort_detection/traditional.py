...
from roborts_msgs.msg import GimbalAngle,PyArmorInfo,GameStatus,GimbalCtrGoal,RobotHeat,RobotAngle,RobotAngleTest,RobotStatus,GameRobotBullet,kalman,YawData,CameraAngle,GimbalCtrAction,GimbalSwingActionGoal
...
class Detector():
    def __init__(self):
        ...
        self.camera_matrix_2_up_old= np.array(([1756.459748365802, -6.862650368968321,427.7240504698368],
                                       [0, 1768.008993030295,131.6003771799422],
                                       [0, 0, 1.0]), dtype=np.double)
        self.dist_coefs_2_up_old = np.array([0.4419,-9.3661,-0.0364,-0.0013,0],dtype=np.double)

        self.camera_matrix_5_up_old= np.array(([1730.070473150964, -6.546114762950307,407.2243869001519],
                                       [0, 1732.183235076287,228.3039372620310],
                                       [0, 0, 1.0]), dtype=np.double)
        self.dist_coefs_5_up_old = np.array([0.0895,-7.4948,-0.0104,-0.0048,0],dtype=np.double)
        ...
        self.chassis_pub=rospy.Publisher('/cmd_vel',Twist,queue_size=1)
        ...
        rospy.Subscriber("/camera_angle",CameraAngle,self.camera_angle_callback)
        self.far=0
        self.my_id=0
        self.loss_enemy=0
        self.find_enemy=0
    ...
    def _shoot(self,shoot_mode, shoot_number,freq):

        #rospy.wait_for_service("cmd_fric_wheel")
        try:
            resp = self._shoot_client.call(shoot_mode, shoot_number,freq)
            ...
            pass
    def camera_angle_callback(self,data):
        ...
        if len(C_lights) != 0 :
            for index in range(len(C_lights)):
                ...
                if (Rect[1][0] / Rect[1][1] >= 1.6 and Rect[1][0] / Rect[1][1] <= 2.8) or (Rect[1][0] / Rect[1][1] >= 0.35714 and Rect[1][0] / Rect[1][1] <= 0.625):
                    ...
                    x_min, x_max, y_min, y_max=self.sortbbox(box)
    ...
    def classic_calcu_angle_car2(self,bbox):
        ...
        if distance >=7500:
            ...
            if (distance <=1650 and self.last_far==1) :
                self.far=1
                pass
            else:
                pitch=pitch-0.042
    ...
    def control_2(self,angle):
        if (angle is not None) :
            ...
            if (self.far-self.last_far)!=0:
                if self.far==1:
                    self.shoot_speed=1255
                elif self.far==2:
                    self.shoot_speed=1270
                elif self.far==3:
            ...
            if abs(self.gimbal_yaw-angle[1])<1.4:
                ...
                if self.gimbal_yaw>0:
                    self.move_cmd.angular.z=2
                    self.chassis_pub.publish(self.move_cmd)
                elif self.gimbal_yaw<=0:
                    self.move_cmd.angular.z=-2
                    self.chassis_pub.publish(self.move_cmd)
            ...
            if self.loss_enemy>100:
                if self.gimbalswing==False:
                    self.gimbal_client.send_goal(GimbalCtrGoal(goal=True,angle=(self.gimbal_yaw*180/3.14)))
    ...
    def control_old_2(self,angle):
        if (angle is not None) :
            ...
            if (self.far-self.last_far)!=0:
                if self.far==1:
                    self.shoot_speed=1220
                elif self.far==2:
                    self.shoot_speed=1220
                elif self.far==3:
                    self.shoot_speed=1230
                elif self.far==4:
                    self.shoot_speed=1250
                elif self.far==5:
                    self.shoot_speed=1265
                elif self.far==6:
                    self.shoot_speed=1275
            self._ctrlinfo_pub.publish(True,False,-angle[1], angle[0])
        ...
        if ROI_find:
            ...
            if len_points!=0 :
                ...
                if len_points==1:
                    ...
                    detector.loss_enemy=0
                    if (angle[2]>5000 and angle[2]<2000):
                        ROI_find=False
                else:
                    ...
                    if min_distance_enemy[2]>5000 and min_distance_enemy[2]<1500:
                        ROI_find=False      
                    detector.loss_enemy=0
                    detector._enemy_pub.publish(0,0,0,0,True)
            else:
                rospy.loginfo('in ROI No detection')
                detector._shoot(0,0,0)
