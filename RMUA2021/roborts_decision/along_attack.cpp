...
//the fourth SelectorNode
    ...
    auto gain_bullet_condition_ = std::make_shared<roborts_decision::PreconditionNode>("gain_bullet_condition", blackboard_ptr_,
        [&]() {
             ...
             &&  (blackboard_ptr_->IsLessBulletThanMate() || blackboard_ptr_->IsMateDie())){  
                return true;
            } else {
                return false;
                }
...
//the fifth  SelectorNode
    ...
    auto enemy_lost_condition_= std::make_shared<roborts_decision::PreconditionNode>("enemy_lost_condition", blackboard_ptr_,
        [&]() {
 		    if (blackboard_ptr_->IsEnemyLost() && blackboard_ptr_->GetCameraLost()){  
                return true;
            } else {
                return false;
                }
...
//the sixth  SelectorNode
    ...
    auto enemy_distance_near_condition_= std::make_shared<roborts_decision::PreconditionNode>("enemy_distance_near_condition", blackboard_ptr_,
        ...
        roborts_decision::AbortType::BOTH); 
        enemy_distance_near_condition_->SetChild(frozen_action_); 
    ...
    auto enemy_distance_far_condition_= std::make_shared<roborts_decision::PreconditionNode>("enemy_distance_far_condition", blackboard_ptr_,
        [&]() {
           //float threa = blackboard_ptr_->GetEnemyThrea();
            //float self_threa = blackboard_ptr_->GetRobotThrea();
            // &&  !(blackboard_ptr_->GetEnemyDistance() <= 2 && abs(self_threa-threa) <= 0.1)
		    if (blackboard_ptr_->GetCameraLost()){ 
                return true;
            } else {
                return false;
    ...
    /*auto enemy_near_not_see_condition_= std::make_shared<roborts_decision::PreconditionNode>("enemy_near_not_see_condition_", blackboard_ptr_,
        [&]() {
		    float threa = blackboard_ptr_->GetEnemyThrea();
            float self_threa = blackboard_ptr_->GetRobotThrea();
		    if (blackboard_ptr_->GetCameraLost() &&  !(blackboard_ptr_->GetEnemyDistance() <= 2 && abs(self_threa-threa) <= 0.1)){ 
                return false;
            } else {
                return true;
                }
        ...
        roborts_decision::AbortType::BOTH); 
        enemy_near_not_see_condition_->SetChild(search_action_);      */
    ...
    enemy_distance_selector_->AddChildren(enemy_distance_far_condition_);
     //enemy_distance_selector_->AddChildren(enemy_near_not_see_condition_);
