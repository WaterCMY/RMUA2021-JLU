...
//#include "action_node/ShootWithDefend.h"
...
int main(int argc, char **argv){
    ...
    // auto chassis_executor_ = new roborts_decision::ChassisExecutor;
    ...
    auto swing_action_=std::make_shared<roborts_decision::DefendAction>(blackboard_ptr_,goal_factory_);                            //
    auto escape_action_=std::make_shared<roborts_decision::EscapeAction>(blackboard_ptr_,goal_factory_);                           //
    auto follow_action_=std::make_shared<roborts_decision::FollowAction>(blackboard_ptr_,goal_factory_);                           //
    auto froze_action_=std::make_shared<roborts_decision::FrozeAction>(blackboard_ptr_,goal_factory_);                             //
    auto gain_blood_action_=std::make_shared<roborts_decision::GainBloodAction>(blackboard_ptr_,goal_factory_);                    //
    auto gain_bullet_action_=std::make_shared<roborts_decision::GainBulletAction>(blackboard_ptr_, goal_factory_);                 //
    ...
    //auto shoot_with_defend_ = std::make_shared<roborts_decision::ShootWithDefend>(blackboard_ptr_, goal_factory_);                 //
    auto turn_to_detected_direction_= std::make_shared<roborts_decision::TurnToDetectedDirection>(blackboard_ptr_, goal_factory_); //                   
    auto wait_buff_refresh_ = std::make_shared<roborts_decision::WaitBuffRefresh>(blackboard_ptr_, goal_factory_);                 //
...
//s4
    ...
    //c9:靠近buff的机器人取buff--------------------------motify number
...
//s5
    ...
    //c13:靠近buff的机器人取buff----------------------------motify number
    ...
    /*快到buff区刷新时间-------------------------也许不用分开写
    auto near_supplier_refresh_time_condition_= std::make_shared<roborts_decision::PreconditionNode>("near_supplier_refresh_time_condition", blackboard_ptr_,
        [&]() {
 		    if (blackboard_ptr_->NearBuff()){  
                return true;
            } else {
                return false;
                }
    ...
    near_supplier_refresh_time_condition_->SetChild(wait_buff_refresh_); 
    */
...
/*自己子弹和血量都充足的条件下---------------也许不需要这个条件节点
    auto with_bullet_and_blood_condition_ = std::make_shared<roborts_decision::PreconditionNode>("with_bullet_and_blood_condition", blackboard_ptr_,
        [&]() {
 		    if (blackboard_ptr_->GetRemainBullets()>20 && blackboard_ptr_->GetRemainHP()>1000){  
                return true;
            } else {
                return false;
                }
...
//s6
    ...
    //with_bullet_and_blood_condtion_->SetChild(actively_approach_selector_);

    //c15:与敌人距离大于3米----------------motify condition
    ...
    //roborts_decision::BehaviorTree root_(game_status_selector_, 100,blackboard_ptr_,mutualboard_ptr_); 
    while(ros::ok()){
        root_.Run();
