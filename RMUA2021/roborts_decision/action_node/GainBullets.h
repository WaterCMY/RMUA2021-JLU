...
namespace roborts_decision{
    ...
    public:
        GainBullets(const Blackboard::Ptr &blackboard_ptr, GoalFactory::GoalFactoryPtr &goal_factory_ptr) :
            ActionNode::ActionNode("gain_bullets_action", blackboard_ptr), goal_factory_ptr_(goal_factory_ptr) {
    ...
    private:
       ...
       virtual BehaviorState Update() {
            ...
            ROS_INFO("gain_bullets_action time:%f",(now_time-start_time_).toSec());
           ...
           /* if(chassis_executor_ptr_->Update() == BehaviorState::SUCCESS){
        ...
        virtual void OnTerminate(BehaviorState state) {
            ...
            if(blackboard_ptr_->GetBulletActive()){
                goal_factory_ptr_->Swing();
                 ROS_INFO("gain_bullets_action   and   swing");
                 }
            /*if(!blackboard_ptr_->GetBulletActive()){
                goal_factory_ptr_->Forward();
                 ROS_INFO("gain_bullets_action   and   Forward");
                 sleep(0.8);
                 }*/
                blackboard_ptr_->SetGainBulletsFlag(true);
