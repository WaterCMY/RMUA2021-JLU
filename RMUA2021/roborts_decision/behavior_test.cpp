...
int main(int argc, char **argv){    
    ...
    ros::Time::init();
    //roslaunch param test
    ros::NodeHandle paramtestnh;
    std::string paramtesta;
    int paramtestb=55;
    paramtestnh.getParam("paramtest_a", paramtesta);
    paramtestnh.getParam("paramtest_b", paramtestb);
    ROS_INFO("paramtest_a:%s",paramtesta.c_str());
    ROS_INFO("paramtest_b:%d",paramtestb);
    ...
    //auto mutual_ptr = std::make_shared<roborts_decision::MutualBoard>(black_ptr); 
    ...
    black_ptr->BuffPose(buff_number);
       ...
       // int game_result_=black_ptr->GetGameResult();
   //std::cout << "游戏的结果是：" <<game_result_<< std::endl;
    ...
    //auto shoot_with_defend = std::make_shared<roborts_decision::ShootWithDefend>(black_ptr, g_factory);                 //
...
//s1
    ...
    //c2:游戏结束----------------modify number
       // auto game_over_condition = std::make_shared<roborts_decision::PreconditionNode>("game_over_condition", black_ptr,
        //[&]() {
 		//    if (/*判断游戏结束*/){  
        //        return true;
        //    } else {
    ...
    //game_over_condition->SetChild(back_boot_area);
    
//s2
    auto game_start_selector=std::make_shared<roborts_decision::SelectorNode>("game_start_selector",black_ptr);
    //game_status_selector->AddChildren(game_not_start_condition);
    //game_status_selector->AddChildren(game_over_condition);
...
//s3
    ...
    //under_punish_selector->AddChildren(gimbal_pubnished_condition);
    //under_punish_selector->AddChildren(chassis_pubnished_condition);
    //under_punish_selector->AddChildren(froze_action);

    
//c4:一方血量不足---------------motify number
...
//s4
    ...
    //c8:血量不足,受到攻击且有击败优势----------------------motify number
    ...
    //c11:血量不足,受到攻击且没有击败优势-----------------------motify number
    ...
    //plan_to_gain_blood_selector->AddChildren(blood_short_with_advantage_condition);
    //plan_to_gain_blood_selector->AddChildren(near_blood_buff_condition);
    //plan_to_gain_blood_selector->AddChildren(near_buff_refresh_time_condition);
    //plan_to_gain_blood_selector->AddChildren(blood_short_and_escape_condition);




//c5:一方子弹不足---------------motify number
...
//s5
    ...
    //c12:子弹不足，受到攻击且有击败优势------------------------------motify number
    ...
    //c14:子弹不足，受到攻击且没有击败优势------------------------------motify number
    ...
    /*快到buff区刷新时间-------------------------也许不用分开写
    auto near_supplier_refresh_time_condition_= std::make_shared<roborts_decision::PreconditionNode>("near_supplier_refresh_time_condition", black_ptr,
        [&]() {
 		    if (black_ptr->NearBuff()){  
                return true;
            } else {
    ...
    near_supplier_refresh_time_condition_->SetChild(wait_buff_refresh); 
    */

    //plan_to_gain_bullet_selector->AddChildren(bullet_short_with_advantage_condition);
    //plan_to_gain_bullet_selector->AddChildren(near_bullet_buff_condition);
    //plan_to_gain_bullet_selector->AddChildren(near_buff_refresh_time_condition); 
    //plan_to_gain_bullet_selector->AddChildren(bullet_short_and_escape_condition);



/*自己子弹和血量都充足的条件下---------------也许不需要这个条件节点
    auto with_bullet_and_blood_condition_ = std::make_shared<roborts_decision::PreconditionNode>("with_bullet_and_blood_condition", black_ptr,
        [&]() {
 		    if (black_ptr->GetRemainBullets()>20 && black_ptr->GetRemainHP()>1000){  
                return true;
            } else {
                return false;
                }
...
//s6
    ...
    //with_bullet_and_blood_condtion_->SetChild(actively_approach_selector);

    //c15:与敌人距离大于2米----------------motify condition
    ...
    //c18:主动进攻时受到攻击且无攻击优势-----------------------motify number
    ...
    //c17:我方每秒掉血量大于一定值-----------------------motify number
    ...
    //actively_approach_selector->AddChildren(actively_with_attacked_condition);
    //actively_approach_selector->AddChildren(actively_swing_condition);
    actively_approach_selector->AddChildren(actively_escape_condition);
    actively_approach_selector->AddChildren(froze_action);
    //actively_approach_selector->AddChildren(actively_search_condition);
    //actively_approach_selector->AddChildren(under_pubulish_and_follow_condition);

    //game_start_selector->AddChildren(under_punish_condition);
    //game_start_selector->AddChildren(one_side_blood_short_condition);
    //game_start_selector->AddChildren(one_side_bullet_short_condition);
    //game_start_selector->AddChildren(actively_approach_selector);
...
//TEST DECK CHASE
    ...
    ros::Rate rate(10);
    while(ros::ok()){
      //int game_status_=black_ptr->GetGameStatus();
 // std::cout << "游戏的状态是：" <<game_status_<< std::endl;
   /*std::cout << "游戏的结果是：" <<black_ptr-> GetGameResult()<< std::endl;
   std::cout << "红色3号存活状态：" <<black_ptr-> GetRed3Survival()<< std::endl;
   std::cout << "红色4号存活状态：" <<black_ptr-> GetRed4Survival()<< std::endl;
   std::cout << "蓝色3号存活状态：" <<black_ptr-> GetBlue3Survival()<< std::endl;
   std::cout << "蓝色4号存活状态：" <<black_ptr-> GetBlue4Survival()<< std::endl;
   std::cout << "GetRedBonusStatus：" <<black_ptr-> GetRedBonusStatus()<< std::endl;
   std::cout << "GetBlueBonusStatus：" <<black_ptr-> GetGameResult()<< std::endl;
   std::cout << "GetSupplierStatus：" <<black_ptr-> GetSupplierStatus2()<< std::endl;
   std::cout << "GetRobotHeat" <<black_ptr-> GetRobotHeat()<< std::endl;
std::cout << "GetRobotBonus" <<black_ptr-> GetRobotBonus()<< std::endl;
std::cout << "GetShootFrequency" <<black_ptr-> GetShootFrequency()<< std::endl;
std::cout << "GetShootSpeed" <<black_ptr-> GetShootSpeed()<< std::endl;*/
        ...
        switch (command) {
            ...
            case '2':
                ...
                //black_ptr->PunishGimbalActionlib();
                root.Run();
                break;
            //gain buff
            case '3':
...
void Command() {
  ...
  while (command != 27) {//esc
    ...
    std::cout << "1: test: back boot area" << std::endl
              ...
              << "a: test: actively_chase_condition"<<std::endl
              << "b: test: deck chase" <<std::endl
       //       << "12: test: go goal" <<std::endl
              << "esc: exit program" << std::endl;
    std::cout << "**************************************************************************************" << std::endl;
    std::cout << "> ";
    std::cin >> command;
    if (command != '1' && command != '2' && command != '3' && command != '4' && command != '5' && command != '6'&& command != '7'&& command != '8' && command != '9' && command != 'a'  && command != 'b' && command != 27) {
