...
ArmorBox::ArmorBox(const LightBar & l_light, const LightBar & r_light) {
	...
	armorRect = cv::boundingRect(armorVertices);
}
...
float ArmorBox::getDislocationY() const {
	float meanLen = (l_light.length + r_light.length) / 2;
	float yDiff = abs(l_light.center.y - r_light.center.y);  //y distance ration y轴方向上的距离比值（y轴距离与灯条平均值的比）
...
float ArmorBox::getLengthRation() const {
    ...
    float lengthDiffRation = length_diff / MIN(l_light.length, r_light.length);
    //printf ("dengtiaobi  %f",lengthDiffRation);
...
{
    int n=0;
	n = (this->getAngleDiff() < armorParam.max_angle_diff &&		// angle difference judge the angleDiff should be less than max_angle_diff 灯条角度差判断，需小于允许的最大角差
		...
		this->getDislocationX() > armorParam.min_x_diff_ratio &&	//dislocation judge: the x and y can not be too near 灯条位置差距 两灯条中心x、y方向差距不可偏小（用比值作为衡量依据）
		this->getDislocationY() < armorParam.max_y_diff_ratio &&
		this->getLengthRation() < armorParam.max_lengthDiff_ratio   // dislocation judge: the x and y can not be too far 灯条位置差距 两灯条中心x、y方向差距不可偏大（用比值作为衡量依据）
		);/*
	MAX(armorRect.height, armorRect.width) < 800 &&
		MIN(armorRect.height, armorRect.width) < 400*/
   // cout <<getAngleDiff()<<" "<<getDeviationAngle()<<" "<<getDislocationX()<<" "<<getDislocationY()<<" "<<getLengthRation()<<" "<<endl;
