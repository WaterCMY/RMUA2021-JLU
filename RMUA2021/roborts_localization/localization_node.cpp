...
bool LocalizationNode::Init() {
  ...
  sentry_sub = nh_.subscribe<roborts_msgs::sg>("sg",30,&LocalizationNode::SentryCallBack,this);
...
void LocalizationNode::SentryCallBack(const roborts_msgs::sg::ConstPtr &sentry_info)
{
  ...
  switch(roborts_id_){
    ...
    if(sentry_info->car3)
    {
    sg_x =  (double)sentry_info->car3_x/100;
    sg_y = (double)sentry_info->car3_y/100;
    ROS_INFO("sg_x= %f,sg_y = %f",sg_x,sg_y);
    }else
    {
      sg_x = -1;
      sg_y =-1;
    ...
    if(sentry_info->car4)
    {
    sg_x =  (double)sentry_info->car4_x/100;
    sg_y = (double)sentry_info->car4_y/100;
    ROS_INFO("sg_x= %f,sg_y = %f",sg_x,sg_y);
    }else
    {
      sg_x = -1;
    ...
    if(sentry_info->car1)
    {
    sg_x =  (double)sentry_info->car1_x/100;
    sg_y = (double)sentry_info->car1_y/100;
    ROS_INFO("sg_x= %f,sg_y = %f",sg_x,sg_y);
    }else
    {
      sg_x = -1;
    ...
    if(sentry_info->car2)
    {
    sg_x =  (double)sentry_info->car2_x/100;
    sg_y = (double)sentry_info->car2_y/100;
    ROS_INFO("sg_x= %f,sg_y = %f",sg_x,sg_y);
    }else
    {
      sg_x = -1;
