...
void Amcl::UpdateUwb(double sg_x,double sg_y) {
 ...
 // std::lock_guard<std::mutex> sample_lock(mutex_);
  ...
  for (int i = 0; i < set_ptr->sample_count; i++) {
    distance[i] = math::EuclideanDistance(sg_x,
                                          sg_y,
                                          set_ptr->samples_vec[i].pose[0],
                                          set_ptr->samples_vec[i].pose[1]);
    if (distance[i] > (3)) {
      set_ptr->samples_vec[i].pose[0] = sg_x + math::RandomGaussianNumByStdDev(uwb_cov_x_);
      set_ptr->samples_vec[i].pose[1] = sg_y + math::RandomGaussianNumByStdDev(uwb_cov_y_);
      set_ptr->samples_vec[i].pose[2] = set_ptr->samples_vec[i].pose[2] + M_PI + math::RandomGaussianNumByStdDev((M_PI / 3.0) * (M_PI / 3.0));
      distance[i] = math::EuclideanDistance(sg_x,
                                            sg_y,
                                            set_ptr->samples_vec[i].pose[0],
                                            set_ptr->samples_vec[i].pose[1]);
...
int Amcl::Update(const Vec3d &pose,
                 ...
                 HypPose &hyp_pose,double sg_x,double sg_y) {
  ...
  if (laser_update_) {
    ...
    //if(sg_x>0 ||sg_y>0){UpdateUwb(sg_x,sg_y);}
