...
#include "roborts_msgs/RobotStatus.h"
		...
		void GameZoneArray2Callback(const roborts_msgs::GameZoneArray::ConstPtr & game_zone_array)
{
		...
		if(F1_zone_status == 5 && F1_zone_active == true){
		  	a = 1;
			  }
	   else if(F2_zone_status == 5 && F2_zone_active == true){
			a = 2;}
     	else if(F3_zone_status == 5 && F3_zone_active == true){
			 a = 3;
			 }
        else if(F4_zone_status == 5 && F4_zone_active == true){
      		a = 4;
			  }
        else if(F5_zone_status == 5 && F5_zone_active == true){
   			a = 5;
			   }
        else if(F6_zone_status == 5 && F6_zone_active == true){
        	a = 6;
			}
 		else {
			a = 7;
    ...
    ros::Time::init();
     ros::init(argc, argv, "DISABLE_SHOOTING");	
	 ros::NodeHandle n;	
	 ros::Subscriber game_zone_array_status_sub_;
     game_zone_array_status_sub_ = n.subscribe<roborts_msgs::GameZoneArray>("game_zone_array_status", 30,GameZoneArray2Callback);
		ros::Publisher pub = n.advertise<geometry_msgs::PoseStamped>("disable_shooting_pose", 10);	
		...
		while(ros::ok()){
			...
			ros::spinOnce();
        	pub.publish(msg2);
			ROS_INFO("set disable shooting pose successed");	
			loop_rate.sleep();
			}
			return 0;
			}
