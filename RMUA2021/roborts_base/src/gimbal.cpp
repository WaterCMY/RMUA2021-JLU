...
void Gimbal::ROS_Init(){
  ...
  ros_sub_cmd_gimbal_angle_ = ros_nh_.subscribe("cmd_gimbal_angle", 1, &Gimbal::GimbalAngleCtrlCallback, this);
    ros_pub_camera_ang_ = ros_nh_.advertise<roborts_msgs::CameraAngle>("camera_angle", 1);
  //ros service
  ...
  //ros_set_shoot_speed_ = ros_nh_.advertiseService("set_shoot_speed", &Gimbal::SetShootSpeedService, this);
  //ros_ctrl_fric_client_ = ros_nh_.serviceClient<roborts_msgs::FricWhl>("cmd_fric_wheel");
  //ros_reset_fric_ = ros_nh_.serviceClient<roborts_msgs::FricWhl>("cmd_fric_wheel");
  //ros_message_init
...
void Gimbal::GimbalInfoCallback(const std::shared_ptr<roborts_sdk::cmd_gimbal_info> gimbal_info){
  ...
  roborts_msgs::CameraAngle camera_ang_msg;
  camera_ang_msg.pitch = gimbal_info->pitch_ecd_angle / 1800.0 * M_PI;
  camera_ang_msg.yaw = gimbal_info->yaw_ecd_angle / 1800.0 * M_PI;
  ros_pub_camera_ang_.publish(camera_ang_msg);
}
...
int Gimbal::SetShootSpeed(int speed_){
     shoot_speed=speed_;
   }

int Gimbal::GetShootSpeed(){
  return shoot_speed;
} 

/*bool Gimbal::SetShootSpeedService(roborts_msgs::ShootSpeed::Request &req,roborts_msgs::ShootSpeed::Response &res){
  SetShootSpeed(req.shootspeed);
  res.received = true;
  srv.request.open = true;
  ros_ctrl_fric_client_.call(srv);
}*/
...
bool Gimbal::CtrlFricWheelService(roborts_msgs::FricWhl::Request &req,
  ...
  SetShootSpeed(req.shootspeed);
  if(req.open){
    fric_speed.left = GetShootSpeed();
    fric_speed.right = GetShootSpeed();
    ROS_INFO("%d",GetShootSpeed());
  } else{
